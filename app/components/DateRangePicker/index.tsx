...
interface Props {
  data: string;
  onSelectDateRangePicker: (item: any) => void
}

function DateRangePickerBase(props: Props) {
  const { styles } = useStyles();
  const {data, onSelectDateRangePicker} = props;
...
const useStyles = createStyles(({ token, css }) => ({
  ...
  container: css`
    ...
    @media (max-width: 1000px) {
      & .rdrCalendarWrapper {
      width: 443px; /* tùy chỉnh chiều rộng */
      height: 454px; /* tùy chỉnh chiều cao */
    }
    }
  `,
